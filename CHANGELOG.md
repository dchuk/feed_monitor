...
6. Push tags and publish the gem (`rbenv exec gem push pkg/feedmon-X.Y.Z.gem`)

## 0.1.0 - 2025-11-08

- First public release of the Feedmon engine with end-to-end feed ingest, scrape orchestration, and Solid Queue dashboards for monitoring, retries, and manual remediation.
...
1. Add `gem "feedmon", "~> 0.1.0"` to your host `Gemfile` and run `rbenv exec bundle install`.
2. Execute `rbenv exec bin/rails railties:install:migrations FROM=feedmon` followed by `rbenv exec bin/rails db:migrate` to copy and run Solid Queue + Feedmon migrations.
3. Review `config/initializers/feedmon.rb` for queue, scraping, and Mission Control settings; adjust the generated defaults to fit your environment.
...
- Converted source fetch, retry, and bulk scrape member actions into nested resources. New controller endpoints:
  - `POST /feedmon/sources/:source_id/fetch` (`Feedmon::SourceFetchesController#create`)
  - `POST /feedmon/sources/:source_id/retry` (`Feedmon::SourceRetriesController#create`)
  - `POST /feedmon/sources/:source_id/bulk_scrape` (`Feedmon::SourceBulkScrapesController#create`)
  Route helpers are now `feedmon.source_fetch_path`, `feedmon.source_retry_path`, and `feedmon.source_bulk_scrape_path`.

### Upgrade Notes

1. Update your host `Gemfile` to the new version and run `rbenv exec bundle install`.
2. Re-run `rbenv exec bin/rails railties:install:migrations FROM=feedmon` followed by `rbenv exec bin/rails db:migrate` to apply new engine migrations (Solid Queue tables remain idempotent).
3. Diff `config/initializers/feedmon.rb` against the generated template to adopt new configuration defaults (queue visibility tweaks, HTTP knobs, mission control toggles).
