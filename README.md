...
Feedmon is a production-ready Rails 8 mountable engine for ingesting, normalising, scraping, and monitoring RSS/Atom/JSON feeds. It ships with a Tailwind-powered admin UI, Solid Queue job orchestration, Solid Cable realtime broadcasting, and an extensible configuration layer so host applications can offer full-stack feed operations without rebuilding infrastructure.
...
- First-class observability through ActiveSupport notifications and `Feedmon::Metrics` counters/gauges
...
### Initial Install
1. Add `gem "feedmon", github: "dchuk/feedmon"` to your Gemfile and run `bundle install`.
2. Install the engine: `bin/rails generate feedmon:install --mount-path=/feedmon` (updates routes, drops the initializer, prints doc links).
3. Copy migrations: `bin/rails railties:install:migrations FROM=feedmon`.
...
7. Boot your app and visit the mount path you chose (default `/feedmon`) to explore the dashboard and confirm Solid Queue metrics render.
...
### Upgrading Feedmon
1. Bump the gem version in your host `Gemfile` and run `bundle install` (or `bundle update feedmon` when targeting a specific release).
2. Re-run `bin/rails railties:install:migrations FROM=feedmon` and then `bin/rails db:migrate` to pick up schema changes.
3. Compare your `config/initializers/feedmon.rb` against the newly generated template for configuration diffs (new queue knobs, HTTP options, etc.).
...
- **Source Lifecycle** – `Feedmon::Fetching::FetchRunner` coordinates advisory locking, fetch execution, retention pruning, and scrape enqueues. Source models store health metrics, failure states, and adaptive scheduling parameters.
- **Item Processing** – `Feedmon::Items::RetentionPruner`, `Feedmon::Scraping::Enqueuer`, and `Feedmon::Scraping::ItemScraper` keep content fresh, ensure deduplicated storage, and capture scrape metadata/logs.
- **Scraping Pipeline** – Adapters inherit from `Feedmon::Scrapers::Base`, merging default + source + invocation settings and returning structured results. The bundled Readability adapter composes `Feedmon::Scrapers::Fetchers::HttpFetcher` and `Feedmon::Scrapers::Parsers::ReadabilityParser`.
- **Realtime Dashboard** – `Feedmon::Dashboard::Queries` batches SQL, caches per-request responses, emits instrumentation (`feedmon.dashboard.*`), and coordinates Turbo broadcasts via Solid Cable.
- **Observability** – `Feedmon::Metrics` tracks counters/gauges for fetches, scheduler runs, and dashboard activity. ActiveSupport notifications (`feedmon.fetch.*`, `feedmon.scheduler.run`, etc.) let you instrument external systems without monkey patches.
- **Extensibility** – `Feedmon.configure` exposes namespaces for queue tuning, HTTP defaults, scraper registry, retention, event callbacks, model extensions, authentication hooks, realtime transports, health thresholds, and job metrics.
...
- Solid Queue becomes the Active Job adapter when the host app still uses the inline `:async` adapter; queue names default to `feedmon_fetch` and `feedmon_scrape` and honour `ActiveJob.queue_name_prefix`.
...
- Retry/backoff behaviour is driven by `Feedmon.configure.fetching`. Fetch completion events and item processors allow you to chain downstream workflows (indexing, notifications, etc.).

## Configuration & API Surface
The generated initializer documents every setting. Key areas:

- Queue namespace/concurrency helpers (`Feedmon.queue_name(:fetch)`)
...
- Precompile assets so Feedmon's bundled CSS/JS outputs are available at runtime.
...
- Monitor gauges/counters emitted by `Feedmon::Metrics` and subscribe to notifications for alerting.
...
Common installation and runtime issues (missing migrations, realtime not streaming, scraping failures, queue visibility gaps) are documented in [docs/troubleshooting.md](docs/troubleshooting.md). When you report bugs, include your `Feedmon::VERSION`, Rails version, configuration snippet, and relevant fetch/scrape logs so we can reproduce quickly.
...
## License
Feedmon is released under the [MIT License](MIT-LICENSE).
