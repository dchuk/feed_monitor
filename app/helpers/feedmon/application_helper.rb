module Feedmon
  module ApplicationHelper
    def feedmon_stylesheet_bundle_tag
      stylesheet_link_tag("feedmon/application", "data-turbo-track": "reload")
    rescue StandardError => error
      log_feedmon_asset_error(:stylesheet, error)
      nil
    end

    def feedmon_javascript_bundle_tag
      javascript_include_tag("feedmon/application", "data-turbo-track": "reload", type: "module")
    rescue StandardError => error
      log_feedmon_asset_error(:javascript, error)
      nil
    end
    ...
    def fetch_interval_bucket_path(bucket, search_params, selected: false)
      ...
      route_helpers = Feedmon::Engine.routes.url_helpers
    ...
    def source_health_actions(source)
      status = source&.health_status.presence || "healthy"
      helpers = Feedmon::Engine.routes.url_helpers
    ...
    def log_feedmon_asset_error(kind, error)
      return unless defined?(Rails) && Rails.respond_to?(:logger) && Rails.logger

      Rails.logger.debug("[Feedmon] Skipping #{kind} bundle include: #{error.message}")
    end
  end
end
