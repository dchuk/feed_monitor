...
module Feedmon
  class SourceHealthCheckJob < ApplicationJob
    ...
    def perform(source_id)
      ...
      result = Feedmon::Health::SourceHealthCheck.new(source: source).call
      broadcast_outcome(source, result)
      result
    rescue StandardError => error
      Rails.logger&.error(
        "[Feedmon::SourceHealthCheckJob] error for source #{source_id}: #{error.class}: #{error.message}"
    ...
    def broadcast_outcome(source, result, error = nil)
      ...
      Feedmon::Realtime.broadcast_toast(message:, level:)
    end
