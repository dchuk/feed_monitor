...
module Feedmon
  class FetchFeedJob < ApplicationJob
    ...
    retry_on Feedmon::Fetching::FetchRunner::ConcurrencyError,
    ...
    def perform(source_id, force: false)
      ...
      Feedmon::Fetching::FetchRunner.new(source: source, force: force).run
    rescue Feedmon::Fetching::FetchError => error
    ...
    def handle_transient_error(source, error)
      ...
      decision = Feedmon::Fetching::RetryPolicy.new(source:, error:, now: Time.current).decision
    ...
    def transient_error?(error)
      error.is_a?(Feedmon::Fetching::FetchError)
    end

    def log_retry_failure(source, original_error, policy_error)
      return unless defined?(Rails) && Rails.respond_to?(:logger) && Rails.logger

      message = "[Feedmon::FetchFeedJob] Failed to schedule retry for source #{source&.id}: " \
