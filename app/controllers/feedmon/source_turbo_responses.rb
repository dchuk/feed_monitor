  ...
  module SourceTurboResponses
      ...
      respond_to do |format|
        format.turbo_stream do
          ...
          responder.replace_row(
            ...
            locals: {
              source: @source,
              item_activity_rates: { @source.id => Feedmon::Analytics::SourceActivityRates.rate_for(@source) },
        ...
        format.html do
          redirect_to feedmon.source_path(@source), notice: message
        end
      end
    end
    ...
    def handle_fetch_failure(error, prefix: "Fetch")
      ...
      respond_to do |format|
        ...
        format.html do
          redirect_to feedmon.source_path(@source), alert: error_message
        end
      end
    end
    ...
    def respond_to_bulk_scrape(result)
      ...
      respond_to do |format|
        ...
        format.html do
          if payload[:message].present?
            redirect_to feedmon.source_path(@source), flash: { payload[:flash_key] => payload[:message] }
          else
            redirect_to feedmon.source_path(@source)
          end
        end
      end
    end
    ...
    def bulk_scrape_flash_payload(result)
      ...
      presenter = Feedmon::Scraping::BulkResultPresenter.new(result:, pluralizer:)
