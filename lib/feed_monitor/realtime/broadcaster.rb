...
module FeedMonitor
  module Realtime
    module Broadcaster
      ...
      SOURCE_INDEX_STREAM = "feed_monitor_sources"
      NOTIFICATION_STREAM = "feed_monitor_notifications"
      ...
      def broadcast_item(item)
        ...
        Turbo::StreamsChannel.broadcast_replace_to(
          ...
          html: render_html = FeedMonitor::ItemsController.render(
      ...
      def broadcast_toast(message:, level: :info, title: nil, delay_ms: 5000)
        ...
        Turbo::StreamsChannel.broadcast_append_to(
          ...
          html: FeedMonitor::ApplicationController.render(
      ...
      def broadcast_source_row(source)
        Turbo::StreamsChannel.broadcast_replace_to(
          ...
          html: FeedMonitor::SourcesController.render(
            partial: "feed_monitor/sources/row",
            locals: {
              source: source,
              activity_rate: FeedMonitor::Analytics::SourceActivityRates.rate_for(source)
            }
          )
        )
      rescue StandardError => error
      ...
      def log_info(stage, **extra)
        ...
        payload = {
          stage: "FeedMonitor::Realtime::Broadcaster##{stage}"
        }.merge(extra.compact)
      ...
      def log_error(context, error)
        Rails.logger.error(
          "[FeedMonitor] Realtime #{context} failed: #{error.class}: #{error.message}"
