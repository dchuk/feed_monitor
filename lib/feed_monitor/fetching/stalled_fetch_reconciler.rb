  ...
  module Fetching
    class StalledFetchReconciler
      ...
      def self.default_stale_after
        if defined?(FeedMonitor::Scheduler::STALE_QUEUE_TIMEOUT)
          FeedMonitor::Scheduler::STALE_QUEUE_TIMEOUT
        else
          10.minutes
        end
      end
      ...
      def stale_sources
        ...
        FeedMonitor::Source.
          ...
          where(FeedMonitor::Source.arel_table[:last_fetch_started_at].lteq(cutoff))
      end
      ...
      def mark_source_failed!(source)
        ...
        FeedMonitor::Realtime.broadcast_source(source) if FeedMonitor::Realtime.respond_to?(:broadcast_source)
      end

      def enqueue_recovery(source)
        FeedMonitor::Fetching::FetchRunner.enqueue(source, force: true)
      end
      ...
      def log_recovery_failure(source, error)
        ...
        Rails.logger.error(
          "[FeedMonitor::Fetching::StalledFetchReconciler] Failed to recover source #{source&.id}: #{error.class}: #{error.message}"
        )
      end
