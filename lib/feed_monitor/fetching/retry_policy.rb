...
module FeedMonitor
  module Fetching
    class RetryPolicy
      ...
      def policy_key
        return :timeout if error.is_a?(FeedMonitor::Fetching::TimeoutError)
        return :connection if error.is_a?(FeedMonitor::Fetching::ConnectionError)

        if error.is_a?(FeedMonitor::Fetching::HTTPError)
        ...
        return :parsing if error.is_a?(FeedMonitor::Fetching::ParsingError)
        return :unexpected if error.is_a?(FeedMonitor::Fetching::UnexpectedResponseError)
