  ...
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      def add_routes_mount
        ...
        route %(mount Feedmon::Engine, at: "#{mount_path}")
      end

      def create_initializer
        initializer_path = "config/initializers/feedmon.rb"
        ...
        template "feedmon.rb.tt", initializer_path
      end
      ...
      def engine_already_mounted?(mount_path)
        ...
        routes_content.include?("mount Feedmon::Engine, at: \"#{mount_path}\"") ||
          routes_content.include?("mount Feedmon::Engine")
      end

      def normalized_mount_path
        raw_path = options.key?(:mount_path) ? options[:mount_path] : "/feedmon"
        path = (raw_path && !raw_path.strip.empty?) ? raw_path.strip : "/feedmon"
