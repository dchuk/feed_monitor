...
require "feedmon/fetching/stalled_fetch_reconciler"
...
module Feedmon
  class Scheduler
    ...
    def run
      payload = { limit: limit }
      recovery = Feedmon::Fetching::StalledFetchReconciler.call(now:, stale_after: STALE_QUEUE_TIMEOUT)
      payload[:stalled_recoveries] = recovery.recovered_source_ids.size
      payload[:stalled_jobs_removed] = recovery.jobs_removed.size

      ActiveSupport::Notifications.instrument("feedmon.scheduler.run", payload) do
        start_monotonic = Feedmon::Instrumentation.monotonic_time
        source_ids = lock_due_source_ids
        payload[:enqueued_count] = source_ids.size

        source_ids.each do |source_id|
          Feedmon::Fetching::FetchRunner.enqueue(source_id)
        end

        payload[:duration_ms] = ((Feedmon::Instrumentation.monotonic_time - start_monotonic) * 1000.0).round(2)
    ...
    def due_for_fetch_predicate
      table = Feedmon::Source.arel_table
