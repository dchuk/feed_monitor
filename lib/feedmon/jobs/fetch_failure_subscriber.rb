...
module Feedmon
  module Jobs
    module FetchFailureCallbacks
      ...
      included do
        after_create :notify_feedmon_of_failure
      end

      private

      def notify_feedmon_of_failure
        Feedmon::Jobs::FetchFailureSubscriber.handle_failed_execution(self)
      end
    end
    ...
    class FetchFailureSubscriber
      ...
      class << self
        ...
        def handle_failed_execution(failed_execution)
          ...
          return unless job
          return unless job.queue_name == Feedmon.queue_name(:fetch)
          ...
          source = Feedmon::Source.find_by(id: source_id)
        ...
        def register_on_load_hook!
          ...
          ActiveSupport.on_load(:solid_queue) do
            Feedmon::Jobs::FetchFailureSubscriber.attach_callbacks!
          end
        ...
        def log_failure(source_id, exception)
          ...
          Rails.logger.error(
            "[Feedmon::Jobs::FetchFailureSubscriber] Failed to handle process failure for source #{source_id.inspect}: #{exception.class}: #{exception.message}"
          )
        end
      end
    end
