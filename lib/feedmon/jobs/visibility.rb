...
module Feedmon
  module Jobs
    module Visibility
      ...
      def setup!
        return if @subscribed || !Feedmon.config.job_metrics_enabled
      ...
      def trackable_job?(job)
        job.class.name.start_with?("Feedmon::")
      rescue StandardError
        false
      end

      def queue_identifier(job)
        (job.queue_name || Feedmon.config.fetch_queue_name).to_s
      end
      ...
      def subscribe_enqueue
        ActiveSupport::Notifications.subscribe("enqueue.active_job") do |_event, _start, _finish, _id, payload|
          ...
          synchronize do
            ...
            Feedmon::Metrics.gauge("jobs_queue_depth_#{queue_name}", state[:depth])
            Feedmon::Metrics.gauge("jobs_last_enqueued_at_#{queue_name}", state[:last_enqueued_at])
          end
        end
      end
      ...
      def subscribe_perform_start
        ActiveSupport::Notifications.subscribe("perform_start.active_job") do |_event, _start, _finish, _id, payload|
          ...
          synchronize do
            ...
            Feedmon::Metrics.gauge("jobs_queue_depth_#{queue_name}", state[:depth])
            Feedmon::Metrics.gauge("jobs_last_started_at_#{queue_name}", state[:last_started_at])
          end
        end
      end
      ...
      def subscribe_perform
        ActiveSupport::Notifications.subscribe("perform.active_job") do |_event, _start, _finish, _id, payload|
          ...
          synchronize do
            ...
            Feedmon::Metrics.gauge("jobs_last_finished_at_#{queue_name}", state[:last_finished_at])
          end
        end
      end
    end
