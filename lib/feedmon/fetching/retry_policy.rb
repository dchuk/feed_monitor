...
module Feedmon
  module Fetching
    class RetryPolicy
      ...
      def policy_key
        return :timeout if error.is_a?(Feedmon::Fetching::TimeoutError)
        return :connection if error.is_a?(Feedmon::Fetching::ConnectionError)

        if error.is_a?(Feedmon::Fetching::HTTPError)
        ...
        return :parsing if error.is_a?(Feedmon::Fetching::ParsingError)
        return :unexpected if error.is_a?(Feedmon::Fetching::UnexpectedResponseError)
