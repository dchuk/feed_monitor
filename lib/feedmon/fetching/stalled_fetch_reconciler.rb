  ...
  module Fetching
    class StalledFetchReconciler
      ...
      def self.default_stale_after
        if defined?(Feedmon::Scheduler::STALE_QUEUE_TIMEOUT)
          Feedmon::Scheduler::STALE_QUEUE_TIMEOUT
        else
          10.minutes
        end
      end
      ...
      def stale_sources
        ...
        Feedmon::Source.
          ...
          where(Feedmon::Source.arel_table[:last_fetch_started_at].lteq(cutoff))
      end
      ...
      def mark_source_failed!(source)
        ...
        Feedmon::Realtime.broadcast_source(source) if Feedmon::Realtime.respond_to?(:broadcast_source)
      end

      def enqueue_recovery(source)
        Feedmon::Fetching::FetchRunner.enqueue(source, force: true)
      end
      ...
      def log_recovery_failure(source, error)
        ...
        Rails.logger.error(
          "[Feedmon::Fetching::StalledFetchReconciler] Failed to recover source #{source&.id}: #{error.class}: #{error.message}"
        )
      end
