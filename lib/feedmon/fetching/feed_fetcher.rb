...
require "feedmon/fetching/retry_policy"
  ...
  module Fetching
    class FeedFetcher
      ...
      def call
        ...
        started_monotonic = Feedmon::Instrumentation.monotonic_time
        result = nil

        Feedmon::Instrumentation.fetch_start(instrumentation_payload)
        ...
        Feedmon::Instrumentation.fetch_finish(instrumentation_payload)
        return result
      end
      ...
      def duration_since(started_monotonic)
        ((Feedmon::Instrumentation.monotonic_time - started_monotonic) * 1000.0).round(2)
      end
      ...
      def connection
        @connection ||= (client || Feedmon::HTTP.client(headers: request_headers))
      end
      ...
      def apply_retry_strategy!(attributes, error, now)
        decision = Feedmon::Fetching::RetryPolicy.new(source:, error:, now:).decision
        ...
        Rails.logger.error(
          "[Feedmon] Failed to apply retry strategy for source #{source.id}: #{policy_error.class} - #{policy_error.message}"
      ...
      def process_feed_entries(feed)
        ...
        Array(feed.entries).each do |entry|
          begin
            result = Feedmon::Items::ItemCreator.call(source:, entry:)
            Feedmon::Events.run_item_processors(source:, entry:, result: result)
            items << result.item
            if result.created?
              created += 1
              created_items << result.item
              Feedmon::Events.after_item_created(item: result.item, source:, entry:, result: result)
            else
              updated += 1
