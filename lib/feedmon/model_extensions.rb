...
module Feedmon
  module ModelExtensions
    class << self
      ...
      def apply_to(entry)
        definition = Feedmon.config.models.for(entry.key)
      ...
      def assign_table_name(entry)
        ...
        desired = "#{Feedmon.table_name_prefix}#{entry.base_table}"
        model_class.table_name = desired
      end

      def apply_concerns(model_class, definition)
        applied = model_class.instance_variable_get(:@_feedmon_extension_concerns) || []
        ...
        model_class.instance_variable_set(:@_feedmon_extension_concerns, applied)
      end
      ...
      def apply_validations(model_class, definition)
        ...
        model_class.instance_variable_set(:@_feedmon_extension_validations, applied_signatures)
        model_class.instance_variable_set(:@_feedmon_extension_validation_filters, applied_filters)
      end
      ...
      def remove_extension_validations(model_class)
        filters = model_class.instance_variable_get(:@_feedmon_extension_validation_filters)
        ...
        model_class.instance_variable_set(:@_feedmon_extension_validations, [])
        model_class.instance_variable_set(:@_feedmon_extension_validation_filters, [])
      end
    end
  end
end
