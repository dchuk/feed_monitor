module Feedmon
  module Assets
    module Bundler
      ...
      def verify!
        ...
        if missing.any?
          ...
          raise MissingBuildError,
            "Feedmon asset build artifacts missing: #{relative_paths.join(', ')}. Run `npm run build` in the engine root to regenerate."
        end
      ...
      def build_artifacts
        [
          engine_root.join("app/assets/builds/feedmon/application.css"),
          engine_root.join("app/assets/builds/feedmon/application.js")
        ]
      end
