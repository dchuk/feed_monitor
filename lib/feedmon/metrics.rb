...
module Feedmon
  module Metrics
    ...
    def setup_subscribers!
      ...
      ActiveSupport::Notifications.subscribe("feedmon.fetch.start") do |_name, _start, _finish, _id, payload|
        increment(:fetch_started_total)
        gauge(:last_fetch_source_id, payload[:source_id]) if payload.key?(:source_id)
      end

      ActiveSupport::Notifications.subscribe("feedmon.fetch.finish") do |_name, start, finish, _id, payload|
      ...
      ActiveSupport::Notifications.subscribe("feedmon.scheduler.run") do |_name, start, finish, _id, payload|
