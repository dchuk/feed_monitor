module Feedmon
  class Engine < ::Rails::Engine
    isolate_namespace Feedmon
    require "feedmon/assets/bundler"
    require "feedmon/jobs/fetch_failure_subscriber"

    def self.table_name_prefix
      Feedmon.config.models.table_name_prefix
    end

    initializer "feedmon.assets" do |app|
      next unless app.config.respond_to?(:assets)

      engine_root = Feedmon::Engine.root
    ...
    initializer "feedmon.assets.sprockets" do |app|
      next unless app.config.respond_to?(:assets)

      manifest_entry = "feedmon_manifest.js"
      app.config.assets.precompile << manifest_entry unless app.config.assets.precompile.include?(manifest_entry)
      app.config.assets.precompile.concat(Feedmon::Engine.asset_precompile_entries)
      app.config.assets.precompile.uniq!
    end

    initializer "feedmon.metrics" do
      Feedmon::Metrics.setup_subscribers!
    end

    initializer "feedmon.dashboard_streams" do
    ...
    initializer "feedmon.jobs" do |app|
      ...
      if defined?(::SolidQueue)
        ...
        if defined?(::SolidQueue::RecurringTask)
          job_class_config = Feedmon.config.recurring_command_job_class
    ...
    class << self
      def asset_precompile_entries
        ...
        asset_roots.flat_map do |_, base_path|
          Dir[base_path.join("feedmon/**/*").to_s].filter_map do |absolute_path|
