  ...
  module Logs
    class Query
      ...
      def initialize(params:)
        @filter_set = Feedmon::Logs::FilterSet.new(params:)
      end

      def call
        pagination_result = Feedmon::Pagination::Paginator.new(
      ...
      def filtered_scope
        scope = Feedmon::LogEntry.includes(:source, :item, :loggable).recent
        ...
        scope = scope.where("feedmon_log_entries.started_at >= ?", filter_set.effective_started_after) if filter_set.effective_started_after
        scope = scope.where("feedmon_log_entries.started_at <= ?", filter_set.started_before) if filter_set.started_before
      ...
      def apply_search(scope, term)
        ...
        scope.
          ...
          where(
            <<~SQL.squish,
              (LOWER(feedmon_log_entries.error_message) LIKE :query) OR
              (LOWER(feedmon_log_entries.error_class) LIKE :query) OR
              (CAST(feedmon_log_entries.http_status AS TEXT) LIKE :query) OR
              (LOWER(feedmon_log_entries.scraper_adapter) LIKE :query) OR
              (LOWER(feedmon_sources.name) LIKE :query) OR
              (LOWER(feedmon_items.title) LIKE :query)
            SQL
            query: normalized
          )
      end
