  ...
  module Scraping
    ...
    class BulkSourceScraper
      ...
      def self.selection_counts(source:, preview_items:, preview_limit: 10)
        ...
        base_scope = Feedmon::Item.active.where(source_id: source.id)
        {
      ...
      def initialize(source:, selection:, preview_limit: DEFAULT_PREVIEW_LIMIT, enqueuer: Feedmon::Scraping::Enqueuer, config: Feedmon.config.scraping)
        @source = source
      ...
      def base_scope
        Feedmon::Item.active.where(source_id: source.id).order(Arel.sql("published_at DESC NULLS LAST, created_at DESC"))
      end

      def without_inflight(scope)
        statuses = Feedmon::Scraping::State::IN_FLIGHT_STATUSES
        column = Feedmon::Item.arel_table[:scrape_status]
        scope.where(column.eq(nil).or(column.not_in(statuses)))
      end

      def self.unscraped_scope
        item_table = Feedmon::Item.arel_table
        failed_statuses = %w[failed partial]
        Feedmon::Item.active.where(
