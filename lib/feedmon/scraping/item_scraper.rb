...
require "feedmon/scraping/item_scraper/persistence"
...
module Feedmon
  module Scraping
    ...
    class ItemScraper
      ...
      def initialize(item:, source: nil, adapter_name: nil, settings: nil, http: Feedmon::HTTP)
        @item = item
        @source = source || item&.source
        @adapter_name = (adapter_name || @source&.scraper_adapter).to_s
        @settings = settings
        @http = http
        @adapter_resolver = Feedmon::Scraping::ItemScraper::AdapterResolver.new(name: @adapter_name, source: @source)
        @persistence = Feedmon::Scraping::ItemScraper::Persistence.new(item: @item, source: @source, adapter_name: @adapter_name)
      end
      ...
      def finalize_result(result)
        ...
        Feedmon::Events.after_item_scraped(result)
        result
      end
      ...
      def log(stage, **extra)
        ...
        payload = {
          stage: "Feedmon::Scraping::ItemScraper##{stage}",
          item_id: item&.id,
          source_id: source&.id,
