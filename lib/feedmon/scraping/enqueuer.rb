...
module Feedmon
  module Scraping
    ...
    class Enqueuer
      ...
      def self.enqueue(item:, source: nil, job_class: Feedmon::ScrapeItemJob, reason: :manual)
        new(item:, source:, job_class:, reason:).enqueue
      end

      def initialize(item:, source: nil, job_class: Feedmon::ScrapeItemJob, reason: :manual)
        @item = item
      ...
      def enqueue
        ...
        item.with_lock do
          ...
          if Feedmon::Scraping::State.in_flight?(item.scrape_status)
          ...
          Feedmon::Scraping::State.mark_pending!(item, broadcast: false, lock: false)
        end
      ...
      def log(stage, item:, **extra)
        ...
        payload = {
          stage: "Feedmon::Scraping::Enqueuer##{stage}",
          item_id: item&.id,
          source_id: source&.id,
          reason: reason
        }.merge(extra.compact)
      ...
      def rate_limit_exhausted?
        limit = Feedmon.config.scraping.max_in_flight_per_source
        return [ false, nil ] unless limit

        in_flight = source.items.where(scrape_status: Feedmon::Scraping::State::IN_FLIGHT_STATUSES).count
