    ...
    class ItemScraper
      ...
      class AdapterResolver
        ...
        def resolve!
          ...
          configured = Feedmon.config.scrapers.adapter_for(name)
          return configured if configured

          constant = resolve_constant
          return constant if constant <= Feedmon::Scrapers::Base
        ...
        def resolve_constant
          Feedmon::Scrapers.const_get(name.camelize)
        end

        def raise_unknown!(message)
          raise Feedmon::Scraping::ItemScraper::UnknownAdapterError, message
        end
      end
    end
  end
