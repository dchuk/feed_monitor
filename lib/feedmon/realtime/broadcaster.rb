...
module Feedmon
  module Realtime
    module Broadcaster
      ...
      SOURCE_INDEX_STREAM = "feedmon_sources"
      NOTIFICATION_STREAM = "feedmon_notifications"
      ...
      def broadcast_item(item)
        ...
        Turbo::StreamsChannel.broadcast_replace_to(
          ...
          html: render_html = Feedmon::ItemsController.render(
            partial: "feedmon/items/details_wrapper",
            locals: { item: item }
          )
        )
        log_info(
      ...
      def broadcast_toast(message:, level: :info, title: nil, delay_ms: 5000)
        ...
        Turbo::StreamsChannel.broadcast_append_to(
          ...
          html: Feedmon::ApplicationController.render(
            partial: "feedmon/shared/toast",
            locals: {
              message: message,
              level: level,
              title: title,
      ...
      def broadcast_source_row(source)
        Turbo::StreamsChannel.broadcast_replace_to(
          ...
          html: Feedmon::SourcesController.render(
            partial: "feedmon/sources/row",
            locals: {
              source: source,
              activity_rate: Feedmon::Analytics::SourceActivityRates.rate_for(source)
            }
          )
        )
      rescue StandardError => error
      ...
      def log_info(stage, **extra)
        ...
        payload = {
          stage: "Feedmon::Realtime::Broadcaster##{stage}"
        }.merge(extra.compact)
      ...
      def register_callback(name, callback)
        callbacks = Feedmon.config.events.callbacks_for(name)
        return if callbacks.include?(callback)

        Feedmon.config.events.public_send(name, callback)
      end

      def log_error(context, error)
        Rails.logger.error(
          "[Feedmon] Realtime #{context} failed: #{error.class}: #{error.message}"
