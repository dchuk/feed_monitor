  ...
  module Realtime
    module Adapter
      ...
      def configure!
        ...
        desired_adapter = Feedmon.config.realtime.adapter
        ...
        if should_replace_adapter?(existing_adapter, desired_adapter)
          apply_configuration(Feedmon.config.realtime.action_cable_config)
        elsif same_adapter?(existing_adapter, desired_adapter)
          merge_defaults(existing_config, Feedmon.config.realtime.action_cable_config)
        end
      end
      ...
      def raise_missing_dependency_error(adapter, error)
        message = <<~ERROR.squish
          Feedmon realtime adapter #{adapter.inspect} could not be loaded: #{error.class}: #{error.message}.
          Ensure the corresponding gem is available or configure `Feedmon.config.realtime.adapter` with a supported backend.
        ERROR
