...
module Feedmon
  module Events
    ...
    def run_item_processors(source:, entry:, result:)
      ...
      Feedmon.config.events.item_processors.each do |processor|
    ...
    def dispatch(event_name, event)
      Feedmon.config.events.callbacks_for(event_name).each do |callback|
    ...
    def log_handler_error(kind, handler, error)
      message = "[Feedmon] #{kind} handler #{handler.inspect} failed: #{error.class}: #{error.message}"
