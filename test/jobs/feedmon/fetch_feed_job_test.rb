...
module Feedmon
  class FetchFeedJobTest < ActiveJob::TestCase
    ...
    test "invokes the fetch runner for the source" do
      ...
      Feedmon::Fetching::FetchRunner.stub(:new, ->(**_kwargs) { runner }) do
        Feedmon::FetchFeedJob.perform_now(source.id)
      end
    ...
    test "retries when a concurrency error occurs" do
      ...
      stub_runner = Class.new do
        ...
        def run
          raise Feedmon::Fetching::FetchRunner::ConcurrencyError, "locked"
        end
      end

      job = Feedmon::FetchFeedJob.new(source.id)

      Feedmon::Fetching::FetchRunner.stub(:new, ->(**_kwargs) { stub_runner.new }) do
    ...
    test "schedules retry using retry policy when a transient fetch error bubbles up" do
      ...
      travel_to Time.zone.local(2025, 10, 12, 10, 0, 0) do
        Feedmon::Fetching::FetchRunner.stub(:new, ->(**_kwargs) { stub_runner.new(error:) }) do
          assert_enqueued_jobs 1 do
            begin
              Feedmon::FetchFeedJob.perform_now(source.id)
            rescue StandardError
      ...
      travel_to Time.zone.local(2025, 10, 12, 12, 0, 0) do
        Feedmon::Fetching::FetchRunner.stub(:new, ->(**_kwargs) { stub_runner.new(error:) }) do
          assert_enqueued_jobs 0 do
            assert_raises(Feedmon::Fetching::TimeoutError) do
              Feedmon::FetchFeedJob.perform_now(source.id)
            end
          end
        end
    ...
    test "no-ops when the source is missing" do
      Feedmon::Fetching::FetchRunner.stub(:new, ->(**) { flunk("runner should not be initialized") }) do
        Feedmon::FetchFeedJob.perform_now(-1)
      end
    ...
    test "executes when force is true even if next fetch is in the future" do
      travel_to Time.zone.local(2025, 10, 30, 12, 0, 0) do
        ...
        Feedmon::Fetching::FetchRunner.stub(:new, ->(**_kwargs) { runner }) do
          Feedmon::FetchFeedJob.perform_now(source.id, force: true)
        end
    ...
    test "executes when next_fetch_at is within early execution leeway" do
      ...
      travel_to(now) do
        leeway = Feedmon::FetchFeedJob::EARLY_EXECUTION_LEEWAY
