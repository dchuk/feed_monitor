...
module Feedmon
  class SourceHealthCheckJobTest < ActiveJob::TestCase
    ...
    test "creates successful health check log" do
      ...
      assert_enqueued_with(job: Feedmon::SourceHealthCheckJob, args: [ source.id ]) do
        Feedmon::SourceHealthCheckJob.perform_later(source.id)
      end

      perform_enqueued_jobs

      log = Feedmon::HealthCheckLog.order(:created_at).last
      ...
      entry = Feedmon::LogEntry.order(:created_at).last
      assert_equal log, entry.loggable
      assert entry.success?, "expected log entry to be successful"
      assert_equal "Feedmon::HealthCheckLog", entry.loggable_type
    end
    ...
    test "records failure details without raising" do
      ...
      Feedmon::Realtime.stub(:broadcast_source, ->(record) { broadcasted_sources << record }) do
        Feedmon::Realtime.stub(:broadcast_toast, ->(**payload) { toasts << payload }) do
          Feedmon::SourceHealthCheckJob.perform_later(source.id)
          perform_enqueued_jobs
        end
      end
    ...
    test "records unexpected errors and broadcasts failure toast" do
      ...
      Feedmon::Health::SourceHealthCheck.stub(:new, ->(**) { failing_service.new }) do
        Feedmon::Realtime.stub(:broadcast_source, ->(record) { broadcasts << record }) do
          Feedmon::Realtime.stub(:broadcast_toast, ->(**payload) { toasts << payload }) do
            Feedmon::SourceHealthCheckJob.perform_now(source.id)
          end
        end
      end
    ...
    test "swallows logging errors when failure recording fails" do
      ...
      Feedmon::Health::SourceHealthCheck.stub(:new, ->(**) { failing_service.new }) do
        Feedmon::HealthCheckLog.stub(:create!, ->(**) { raise StandardError, "log failure" }) do
