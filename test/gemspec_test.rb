...
class GemspecTest < ActiveSupport::TestCase
  def setup
    @spec = Gem::Specification.load(File.expand_path("../feedmon.gemspec", __dir__))
    assert_not_nil @spec, "Expected gemspec to load"
  end

  test "gemspec metadata is release ready" do
    assert_equal "Feedmon engine for ingesting, scraping, and monitoring RSS/Atom/JSON feeds", @spec.summary
    assert_includes @spec.description, "mountable Rails 8 engine"

      assert_equal "https://github.com/dchuk/feedmon", @spec.metadata["source_code_uri"]
      assert_equal "https://github.com/dchuk/feedmon/blob/main/CHANGELOG.md", @spec.metadata["changelog_uri"]
      assert_equal "https://github.com/dchuk/feedmon#readme", @spec.metadata["documentation_uri"]
