...
module HostAppHarness
  ...
  def append_engine_to_gemfile!(root)
    ...
    File.open(gemfile, "a") do |file|
      file.puts
      file.puts %(gem "feedmon", path: "#{gem_path}")
    end
  end
  ...
  def ensure_ruby_version!
    ...
    if rbenv_available?
      raise <<~MESSAGE
        Feedmon requires Ruby #{TARGET_RUBY_VERSION}. Detected #{::RUBY_VERSION}.
        Please install #{TARGET_RUBY_VERSION} (e.g., via rbenv: `rbenv install #{TARGET_RUBY_VERSION}`) and re-run the test suite.
      MESSAGE
    else
      Kernel.warn <<~MESSAGE
        Feedmon expected Ruby #{TARGET_RUBY_VERSION} but detected #{::RUBY_VERSION}.
  ...
  def ensure_rails_available!
    ...
    raise <<~MESSAGE
      Feedmon's host app harness expects Rails >= 8.0.3 to be installed.
  ...
  def ensure_sqlite_json_patch!(root)
    ...
    File.write(initializer_path, <<~RUBY)
      ...
      ActiveSupport.on_load(:active_record) do
        ...
        unless defined?(Feedmon::SQLiteJsonbShim)
          module Feedmon
        ...
        targets.each do |target|
          next if target < Feedmon::SQLiteJsonbShim

          target.prepend(Feedmon::SQLiteJsonbShim)
        end
      end
    RUBY
  end
