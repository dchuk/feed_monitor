...
module Feedmon
  class SourcesTest < ApplicationSystemTestCase
    ...
    test "managing a source end to end" do
      ...
      source = Feedmon::Source.find_by!(feed_url: "https://example.com/feed")
      assert_equal "UI Source", source.name
      assert_current_path feedmon.source_path(Feedmon::Source.last)
      assert_text "Retention Policy Active"

      source = Feedmon::Source.last
      ...
      visit feedmon.source_path(source)
      ...
      assert_current_path feedmon.source_path(source)
      assert_text "Updated Source"

      click_link "Sources"
      assert_current_path feedmon.sources_path
      assert_text "Updated Source"
      ...
      visit feedmon.source_path(source)
      delete_form = find("form[action='#{feedmon.source_path(source)}']")
      ...
      refute Feedmon::Source.exists?(source.id)
    ...
    test "sources table supports sorting and dropdown actions" do
      ...
      within "turbo-frame#feedmon_sources_table thead" do
        click_link "Source"
      end

      assert_source_order [ "Alpha Feed", "Zeta Feed" ]
      within "turbo-frame#feedmon_sources_table thead th[data-sort-column='name']" do
        assert_text "▲"
      end
      ...
      within "turbo-frame#feedmon_sources_table" do
        first_row = find("tbody tr:first-child")
        within first_row do
          name_link = find("td:first-child a", text: "Zeta Feed")
          assert_equal feedmon.source_path(newer), URI.parse(name_link[:href]).path
    ...
    test "source dropdown links navigate to view and edit pages" do
      ...
      assert_current_path feedmon.edit_source_path(source)
    end
    ...
    test "deleting a source from the index removes the row and refreshes heatmap" do
      ...
      assert Feedmon::Source.exists?(keep_source.id), "expected other sources to remain"
    ...
    test "bulk scrape modal enqueues selections and handles empty scopes" do
      ...
      clear_enqueued_jobs
      source.reload
      assert_equal 3, Feedmon::Item.where(source: source, scrape_status: "pending").count
    ...
    test "failing source dropdown enqueues recovery actions and shows processing state" do
      ...
      assert_equal Feedmon::SourceHealthCheckJob, enqueued_jobs.last[:job]

      with_inline_jobs { perform_enqueued_jobs }

      visit feedmon.logs_path(log_type: "health_check")
