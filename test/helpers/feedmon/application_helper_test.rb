...
module Feedmon
  class ApplicationHelperTest < ActionView::TestCase
    include Feedmon::ApplicationHelper

    test "feedmon_stylesheet_bundle_tag renders link tag" do
      tag = feedmon_stylesheet_bundle_tag

      assert_includes tag, "feedmon/application"
      assert_includes tag, "rel=\"stylesheet\""
    end

    test "feedmon_stylesheet_bundle_tag logs and returns nil on error" do
      Rails.stub(:logger, Logger.new(IO::NULL)) do
        stub(:stylesheet_link_tag, ->(*_args) { raise StandardError, "boom" }) do
          assert_nil feedmon_stylesheet_bundle_tag
        end
      end
    end

    test "feedmon_javascript_bundle_tag renders module script" do
      tag = feedmon_javascript_bundle_tag

      assert_includes tag, "feedmon/application"
      assert_includes tag, "type=\"module\""
    end

    test "feedmon_javascript_bundle_tag logs and returns nil on error" do
      Rails.stub(:logger, Logger.new(IO::NULL)) do
        stub(:javascript_include_tag, ->(*_args) { raise StandardError, "boom" }) do
          assert_nil feedmon_javascript_bundle_tag
        end
      end
    end
    ...
    test "fetch_interval_bucket_path builds query string when filters present" do
      ...
      assert_equal "/feedmon/sources", uri.path
      ...
      assert_equal "/feedmon/sources", cleared_uri.path
    ...
    test "source_health_badge returns healthy styling" do
      source = Feedmon::Source.new(health_status: "healthy")
    ...
    test "source_health_badge highlights declining status" do
      source = Feedmon::Source.new(health_status: "declining")
    ...
    test "source_health_badge highlights improving status" do
      source = Feedmon::Source.new(health_status: "improving")
    ...
    test "source_health_badge indicates auto paused" do
      source = Feedmon::Source.new(health_status: "auto_paused")
    ...
    test "item_scrape_status_badge shows scraped label for success" do
      source = Feedmon::Source.new(scraping_enabled: true)
      item = Feedmon::Item.new(source:, guid: "status-success", url: "https://example.com/success", scrape_status: "success")
    ...
    test "item_scrape_status_badge shows pending spinner" do
      source = Feedmon::Source.new(scraping_enabled: true)
      item = Feedmon::Item.new(source:, guid: "status-pending", url: "https://example.com/pending", scrape_status: "pending")
    ...
    test "item_scrape_status_badge reports disabled when source scraping disabled" do
      source = Feedmon::Source.new(scraping_enabled: false)
      item = Feedmon::Item.new(source:, guid: "status-disabled", url: "https://example.com/disabled")
    ...
    test "item_scrape_status_badge treats never scraped items as not scraped" do
      source = Feedmon::Source.new(scraping_enabled: true)
      item = Feedmon::Item.new(source:, guid: "status-never", url: "https://example.com/never")
