...
module FeedMonitor
  class ApplicationHelperTest < ActionView::TestCase
    ...
    test "feed_monitor_stylesheet_bundle_tag renders link tag" do
      tag = feed_monitor_stylesheet_bundle_tag

      assert_includes tag, "feed_monitor/application"
      assert_includes tag, "rel=\"stylesheet\""
    end

    test "feed_monitor_stylesheet_bundle_tag logs and returns nil on error" do
      Rails.stub(:logger, Logger.new(IO::NULL)) do
        stub(:stylesheet_link_tag, ->(*_args) { raise StandardError, "boom" }) do
          assert_nil feed_monitor_stylesheet_bundle_tag
        end
      end
    end

    test "feed_monitor_javascript_bundle_tag renders module script" do
      tag = feed_monitor_javascript_bundle_tag

      assert_includes tag, "feed_monitor/application"
      assert_includes tag, "type=\"module\""
    end

    test "feed_monitor_javascript_bundle_tag logs and returns nil on error" do
      Rails.stub(:logger, Logger.new(IO::NULL)) do
        stub(:javascript_include_tag, ->(*_args) { raise StandardError, "boom" }) do
          assert_nil feed_monitor_javascript_bundle_tag
        end
      end
    end
    ...
    test "fetch_interval_bucket_path builds query string when filters present" do
      ...
      assert_equal "/feed_monitor/sources", uri.path
      ...
      assert_equal "/feed_monitor/sources", cleared_uri.path
