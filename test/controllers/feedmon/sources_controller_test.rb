...
module Feedmon
  class SourcesControllerTest < ActionDispatch::IntegrationTest
    ...
    test "sanitizes attributes when creating a source" do
      ...
      assert_redirected_to "/feedmon/sources/#{source.id}"
    ...
    test "destroy removes source and dependents via turbo stream" do
      ...
      assert_difference [
        -> { Feedmon::Source.count },
        -> { Feedmon::Item.count },
        -> { Feedmon::FetchLog.count },
        -> { Feedmon::ScrapeLog.count }
      ], -1 do
        delete feedmon.source_path(source), params: { q: { "name_or_feed_url_or_website_url_cont" => "Keep" } }, as: :turbo_stream
      end

      assert_response :success
      assert_equal "text/vnd.turbo-stream.html", response.media_type
      assert_includes response.body, %(<turbo-stream action="remove" target="#{dom_id(source, :row)}">)
      assert_includes response.body, %(<turbo-stream action="replace" target="feedmon_sources_heatmap">)
      assert_includes response.body, %(<turbo-stream action="append" target="feedmon_notifications">)

      assert Feedmon::Source.exists?(keep_source.id), "expected other sources to remain"
      refute Feedmon::Item.exists?(item.id), "expected associated items to be deleted"
    end
    ...
    test "destroy turbo stream includes redirect when redirect_to provided" do
      ...
      delete feedmon.source_path(source),
        params: { redirect_to: feedmon.sources_path },
        as: :turbo_stream
      ...
      assert_includes response.body, %(url="#{feedmon.sources_path}")
