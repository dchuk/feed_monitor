...
# Feedmon engine configuration for the dummy host app.
...
Feedmon.configure do |config|
  ...
  # config.http.headers = { "X-Feedmon" => "dummy" }
  ...
  config.models.source.include_concern "DummyFeedmon::SourceExtensions"
  config.models.source.validate :enforce_testing_notes_length
  config.models.source.validate do |record|
    next unless record.is_a?(Feedmon::SponsoredSource)
