...
skip_coverage_flag = ENV.fetch("FEEDMON_SKIP_COVERAGE", "")
skip_coverage =
...
if (ENV["CI"] || ENV["COVERAGE"]) && !skip_coverage
  ...
  command_name = ENV.fetch("SIMPLECOV_COMMAND_NAME", "feedmon:test")
  SimpleCov.command_name command_name
  SimpleCov.start "rails" do
    enable_coverage :branch
    refuse_coverage_drop :line if command_name == "feedmon:test"
    add_filter %r{^/test/}
  end
...
class ActiveSupport::TestCase
  worker_count = ENV.fetch("FEEDMON_TEST_WORKERS", :number_of_processors)
  ...
  def create_source!(attributes = {})
    ...
    source = Feedmon::Source.new(defaults.merge(attributes))
