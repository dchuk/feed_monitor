  ...
  module Dashboard
    class QueriesTest < ActiveSupport::TestCase
      ...
      test "recent_activity returns events without route data and caches by limit" do
        ...
        assert events.all? { |event| event.is_a?(Feedmon::Dashboard::RecentActivity::Event) }
      ...
      test "stats instrumentation records duration metrics" do
        ...
        subscriber = ActiveSupport::Notifications.subscribe("feedmon.dashboard.stats") do |*args|
        ...
        assert Feedmon::Metrics.gauge_value(:dashboard_stats_duration_ms), "expected metrics gauge for duration"
        assert Feedmon::Metrics.gauge_value(:dashboard_stats_last_run_at_epoch), "expected metrics gauge for last run timestamp"
      ...
      test "job_metrics maps summaries to configured queue roles" do
        queries = Feedmon::Dashboard::Queries.new
        fetch_summary = Feedmon::Jobs::SolidQueueMetrics::QueueSummary.new(
        ...
        Feedmon::Jobs::SolidQueueMetrics.stub(:call, { Feedmon.queue_name(:fetch) => fetch_summary }) do
          ...
          assert_equal Feedmon.queue_name(:scrape), scrape_row[:queue_name]
      ...
      test "upcoming_fetch_schedule caches grouped sources" do
        ...
        Feedmon::Source.where(id: source.id).update_all(next_fetch_at: Time.current + 5.hours)
      ...
      test "recent_activity caches results per limit key" do
        ...
        Feedmon::Dashboard::Queries::RecentActivityQuery.stub(:new, ->(**kwargs) {
      ...
      test "recent_activity_query sanitizes SQL with the provided limit" do
        query = Feedmon::Dashboard::Queries::RecentActivityQuery.new(limit: 3)
