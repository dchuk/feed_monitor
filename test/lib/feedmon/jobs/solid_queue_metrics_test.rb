  ...
  module Jobs
    class SolidQueueMetricsTest < ActiveSupport::TestCase
      ...
      test "aggregates counts and timestamps per queue" do
        ...
        scrape_queue = Feedmon.queue_name(:scrape)
        ...
        travel_to(base_time + 25.minutes) do
          SolidQueue::RecurringTask.create!(
            key: "feedmon_test_#{SecureRandom.hex(4)}",
            schedule: "every minute",
            class_name: "Feedmon::ScheduleFetchesJob",
            queue_name: fetch_queue
          )
          SolidQueue::Pause.create!(queue_name: fetch_queue)
        end

        metrics = Feedmon::Jobs::SolidQueueMetrics.call(queue_names: [ fetch_queue, scrape_queue ])
