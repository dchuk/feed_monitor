  ...
  module Scraping
    class BulkSourceScraperTest < ActiveSupport::TestCase
      ...
      test "selection helpers normalize input for UI" do
        assert_equal "current view", Feedmon::Scraping::BulkSourceScraper.selection_label("current")
        assert_equal "unscraped items", Feedmon::Scraping::BulkSourceScraper.selection_label("UNSCRAPED")
        assert_equal "current view", Feedmon::Scraping::BulkSourceScraper.selection_label("invalid")

        normalized = Feedmon::Scraping::BulkSourceScraper.normalize_selection(" All ")
      ...
      test "enqueues scraping for current items preview" do
        ...
        assert_enqueued_jobs 3 do
          result = Feedmon::Scraping::BulkSourceScraper.new(
            source:,
            selection: :current,
            preview_limit: 10
          ).call
      ...
      test "selection counts ignore association cache limits" do
        ...
        counts = Feedmon::Scraping::BulkSourceScraper.selection_counts(
          source:,
