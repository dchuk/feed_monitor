    ...
    class SchedulerTest < ActiveSupport::TestCase
      ...
      test "enqueues scraping jobs for auto-scrape sources" do
        ...
        assert_difference -> { enqueued_jobs.size }, 2 do
          Feedmon::Scraping::Scheduler.run(limit: 10)
        end

        assert_enqueued_with(job: Feedmon::ScrapeItemJob, args: [ item_one.id ])
        assert_enqueued_with(job: Feedmon::ScrapeItemJob, args: [ item_two.id ])
      end
      ...
      test "respects the provided limit" do
        ...
        assert_difference -> { enqueued_jobs.size }, 1 do
          Feedmon::Scraping::Scheduler.run(limit: 1)
        end

        assert_enqueued_with(job: Feedmon::ScrapeItemJob, args: [ first_item.id ])
      end
