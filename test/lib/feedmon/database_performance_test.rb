...
module Feedmon
  class DatabasePerformanceTest < ActiveSupport::TestCase
    ...
    test "critical tables expose required indexes" do
      assert_has_index(:feedmon_sources, %w[feed_url])
      assert_has_index(:feedmon_sources, %w[next_fetch_at])
      assert_has_index(:feedmon_sources, %w[created_at])

      assert_has_index(:feedmon_items, %w[source_id published_at created_at])

      assert_has_index(:feedmon_scrape_logs, %w[started_at])
    end
    ...
    test "retention pruner soft deletes update counter cache with a single statement" do
      ...
      update_statements = capture_sql do
        travel_to Time.zone.local(2025, 10, 5, 12, 0, 0) do
          Feedmon::Items::RetentionPruner.call(source:, strategy: :soft_delete)
        end
      end.select { |sql| sql.include?('UPDATE "feedmon_sources"') && sql.include?('"items_count"') }
      ...
      assert_equal 3, Feedmon::Item.only_deleted.count
    end
