  ...
  module Fetching
    class FeedFetcherTest < ActiveSupport::TestCase
      test "continues processing when an item creation fails" do
        ...
        singleton = Feedmon::Items::ItemCreator.singleton_class
        call_count = 0
        ...
        begin
          VCR.use_cassette("feedmon/fetching/rss_success") do
      ...
      test "fetches an RSS feed and records log entries" do
        ...
        assert_equal result.feed.entries.size, Feedmon::Item.where(source: source).count
      ...
      test "parses rss atom and json feeds via feedjira" do
        ...
        feeds.each do |format, data|
          ...
          result = nil
          VCR.use_cassette("feedmon/fetching/#{format}_success") do
            result = FeedFetcher.new(source: source, jitter: ->(_) { 0 }).call
          end
      ...
      test "records timeout failures and emits failure notifications" do
        ...
        assert_kind_of Feedmon::Fetching::TimeoutError, result.error
        ...
        assert_equal "Feedmon::Fetching::TimeoutError", log.error_class
        ...
        assert_equal "Feedmon::Fetching::TimeoutError", payload[:error_class]
      ...
      test "records http failures with status codes" do
        ...
        assert_equal :failed, result.status
        assert_kind_of Feedmon::Fetching::HTTPError, result.error
        ...
        assert_equal "Feedmon::Fetching::HTTPError", log.error_class
      ...
      test "records parsing failures when feed is malformed" do
        ...
        assert_equal :failed, result.status
        assert_kind_of Feedmon::Fetching::ParsingError, result.error
        ...
        refute log.success
        assert_equal 200, log.http_status
        assert_equal "Feedmon::Fetching::ParsingError", log.error_class
      ...
      test "increases interval when feed content unchanged" do
        ...
        source.reload
        expected_minutes = (45 * Feedmon::Fetching::FeedFetcher::INCREASE_FACTOR).round
        assert_equal expected_minutes, source.fetch_interval_minutes
        expected_seconds = 45 * 60 * Feedmon::Fetching::FeedFetcher::INCREASE_FACTOR
      ...
      test "respects min and max interval bounds" do
        ...
        min_minutes = (Feedmon::Fetching::FeedFetcher::MIN_FETCH_INTERVAL / 60.0).round
        ...
        max_minutes = (Feedmon::Fetching::FeedFetcher::MAX_FETCH_INTERVAL / 60.0).round
      ...
      test "increases interval and sets backoff on failure" do
        ...
        source.reload
        assert_equal 1, source.failure_count
        expected_minutes = (60 * Feedmon::Fetching::FeedFetcher::FAILURE_INCREASE_FACTOR).round
        assert_equal expected_minutes, source.fetch_interval_minutes
        assert_equal source.next_fetch_at, source.backoff_until
      ensure
        travel_back
