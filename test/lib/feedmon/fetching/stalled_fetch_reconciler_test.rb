...
module Feedmon
  module Fetching
    class StalledFetchReconcilerTest < ActiveSupport::TestCase
      STALE_THRESHOLD = Feedmon::Scheduler::STALE_QUEUE_TIMEOUT
      ...
      test "recovers stale fetching source, discards failed executions, and re-enqueues fetch" do
        ...
        with_queue_adapter(:solid_queue) do
          result = Feedmon::Fetching::StalledFetchReconciler.call(now:, stale_after: STALE_THRESHOLD)
        end
        ...
        matching_jobs = SolidQueue::Job.where(queue_name: Feedmon.queue_name(:fetch)).
    ...
    test "default stale_after uses scheduler timeout when not provided" do
      assert_equal STALE_THRESHOLD, Feedmon::Fetching::StalledFetchReconciler.send(:default_stale_after)
    end

    test "default stale_after falls back to ten minutes when scheduler timeout missing" do
      scheduler = Feedmon::Scheduler
      original = Feedmon::Scheduler::STALE_QUEUE_TIMEOUT
      scheduler.send(:remove_const, :STALE_QUEUE_TIMEOUT)

      assert_equal 10.minutes, Feedmon::Fetching::StalledFetchReconciler.send(:default_stale_after)
    ensure
    ...
    test "recover_source logs and swallows unexpected errors" do
      ...
      reconciler = Feedmon::Fetching::StalledFetchReconciler.send(:new, now:, stale_after: STALE_THRESHOLD)
    ...
    test "jobs_supported? returns false when Solid Queue tables are unavailable" do
      reconciler = Feedmon::Fetching::StalledFetchReconciler.send(:new, now: Time.current, stale_after: 1.minute)
