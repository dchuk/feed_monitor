  ...
  module Fetching
    class FetchRunnerTest < ActiveSupport::TestCase
      ...
      test "enqueues scrape jobs for newly created items when auto scrape is enabled" do
        ...
        processing = Feedmon::Fetching::FeedFetcher::EntryProcessingResult.new(
          created: 1,
          updated: 0,
          failed: 0,
          items: [ item ],
        ...
        result = Feedmon::Fetching::FeedFetcher::Result.new(
          status: :fetched,
          feed: nil,
          response: nil,
          body: nil,
        ...
        assert_enqueued_with(job: Feedmon::ScrapeItemJob, args: [ item.id ]) do
      ...
      test "enqueue marks source as queued" do
        ...
        Feedmon::Realtime.stub :broadcast_source, nil do
          Feedmon::FetchFeedJob.stub :perform_later, nil do
      ...
      test "run updates fetch status lifecycle for successful fetch" do
        ...
        stub_fetcher = Class.new do
          ...
          define_method(:call) do
            Feedmon::Fetching::FeedFetcher::Result.new(status: :fetched)
          end
        end
      ...
      test "does not enqueue scrape jobs when auto scrape is disabled" do
        ...
        assert_no_enqueued_jobs only: Feedmon::ScrapeItemJob do
          FetchRunner.new(source:, fetcher_class: stub_fetcher).run
        end
      ...
      test "raises concurrency error when advisory lock acquisition fails" do
        ...
        failing_lock_class = Class.new do
          ...
          def with_lock
            raise Feedmon::Fetching::AdvisoryLock::NotAcquiredError, "busy"
          end
        end
      ...
      test "invokes retention pruner after each fetch run" do
        ...
        stub_fetcher = Class.new do
          ...
          define_method(:call) do
            Feedmon::Fetching::FeedFetcher::Result.new(status: :not_modified)
          end
        end
      ...
      test "delegates completion to injected handlers" do
        ...
        result = Feedmon::Fetching::FeedFetcher::Result.new(status: :fetched, item_processing: processing)
      ...
      test "schedules retry according to timeout policy" do
        source = create_source
        error = Feedmon::Fetching::TimeoutError.new("timeout")
        ...
        travel_to Time.zone.parse("2025-10-11 09:00:00 UTC") do
          ...
          assert_equal Feedmon::FetchFeedJob, enqueued[:job]
      ...
      def empty_processing_result
        Feedmon::Fetching::FeedFetcher::EntryProcessingResult.new(
          created: 0,
          updated: 0,
          failed: 0,
          items: [],
      ...
      def build_timeout_failure_fetcher(error)
        ...
        Class.new do
          ...
          define_method(:call) do
            now = Time.current
            decision = Feedmon::Fetching::RetryPolicy.new(source: @source, error: error, now: now).decision
            context.send(:apply_decision!, @source, decision, now)
            Feedmon::Fetching::FeedFetcher::Result.new(
              status: :failed,
              error: error,
