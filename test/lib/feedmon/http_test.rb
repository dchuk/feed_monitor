...
module Feedmon
  class HTTPTest < ActiveSupport::TestCase
    setup do
      ...
      @connection = Feedmon::HTTP.client
    end
    ...
    test "configures faraday connection with timeouts, redirects, and compression" do
      ...
      assert_equal Feedmon::HTTP::DEFAULT_TIMEOUT, @connection.options.timeout
      assert_equal Feedmon::HTTP::DEFAULT_OPEN_TIMEOUT, @connection.options.open_timeout

      follow_redirects = handlers.find { |handler| handler.klass == Faraday::FollowRedirects::Middleware }

      refute_nil follow_redirects
      assert_equal Feedmon::HTTP::DEFAULT_MAX_REDIRECTS,
    ...
    test "adds retry middleware with exponential backoff" do
      ...
      assert_equal Feedmon::HTTP::RETRY_STATUSES, options[:retry_statuses]
    end

    test "can disable retry middleware" do
      connection = Feedmon::HTTP.client(retry_requests: false)
    ...
    test "supports proxy arguments" do
      proxy_connection = Feedmon::HTTP.client(proxy: "http://proxy.test:8080")
    ...
    test "allows overriding headers while preserving defaults" do
      custom = Feedmon::HTTP.client(headers: { "User-Agent" => "Feedmon/Test" })

      assert_equal "Feedmon/Test", custom.headers["User-Agent"]
    ...
    test "merges configured headers and user agent" do
      Feedmon.configure do |config|
        config.http.user_agent = -> { "Feedmon/Custom" }
        config.http.headers = {
      ...
      client = Feedmon::HTTP.client(headers: { "X-Request-ID" => "abc123" })

      assert_equal "Feedmon/Custom", client.headers["User-Agent"]
