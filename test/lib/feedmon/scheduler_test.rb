...
module Feedmon
  class SchedulerTest < ActiveSupport::TestCase
    ...
    test "enqueues fetch jobs for sources due for fetch" do
      ...
      travel_to(now) do
        assert_difference -> { enqueued_jobs.size }, 2 do
          Feedmon::Scheduler.run(limit: nil)
        end
      end
    ...
    test "uses skip locked when selecting due sources" do
      ...
      sql = capture_sql do
        Feedmon::Scheduler.run(limit: 1, now: now)
      end
    ...
    test "returns number of sources enqueued" do
      ...
      travel_to(now) do
        assert_equal 1, Feedmon::Scheduler.run(limit: nil)
      end
    end
    ...
    test "invokes stalled fetch reconciler before scheduling sources" do
      ...
      Feedmon::Fetching::StalledFetchReconciler.stub(:call, ->(**args) do
        called = true
        assert_equal now, args[:now]
        assert_equal Feedmon::Scheduler::STALE_QUEUE_TIMEOUT, args[:stale_after]
        stubbed_result
      end) do
        travel_to(now) do
          Feedmon::Scheduler.run(limit: nil, now: now)
        end
      end
    ...
    test "re-enqueues sources stuck in queued state beyond timeout" do
      ...
      stale_time = now - (Feedmon::Scheduler::STALE_QUEUE_TIMEOUT + 5.minutes)
    ...
    test "fetch_status_predicate builds correct arel conditions" do
      now = Time.current
      scheduler = Feedmon::Scheduler.new(limit: 10, now: now)
      ...
      queued_stale.update_columns(updated_at: now - (Feedmon::Scheduler::STALE_QUEUE_TIMEOUT + 2.minutes))

      # Apply the predicate and verify results
      ids = Feedmon::Source.where(predicate).pluck(:id)
    ...
    test "includes stale fetching sources in eligible predicate" do
      ...
      stale_fetching = create_source(
        ...
        last_fetch_started_at: now - (Feedmon::Scheduler::STALE_QUEUE_TIMEOUT + 5.minutes)
      )
      ...
      Feedmon::Fetching::StalledFetchReconciler.stub(:call, Feedmon::Fetching::StalledFetchReconciler::Result.new(recovered_source_ids: [], jobs_removed: [], executed_at: now)) do
        travel_to(now) do
          Feedmon::Scheduler.run(limit: nil, now: now)
        end
      end
    ...
    test "instruments scheduler runs and updates metrics" do
      ...
      events = []
      subscription = ActiveSupport::Notifications.subscribe("feedmon.scheduler.run") do |*args|
        events << args.last
      end
