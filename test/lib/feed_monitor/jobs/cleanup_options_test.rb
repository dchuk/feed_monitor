  ...
  module Jobs
    class CleanupOptionsTest < ActiveSupport::TestCase
      test "normalize returns empty hash when nil" do
        assert_equal({}, FeedMonitor::Jobs::CleanupOptions.normalize(nil))
      end

      test "normalize symbolises keys" do
        options = FeedMonitor::Jobs::CleanupOptions.normalize("now" => "2025-10-12 10:00:00")
      ...
      test "resolve_time parses string with timezone awareness" do
        travel_to Time.zone.local(2025, 10, 12, 12, 0, 0) do
          time = FeedMonitor::Jobs::CleanupOptions.resolve_time("2025-10-11 09:30:00")
      ...
      test "resolve_time falls back to current time when parsing fails" do
        travel_to Time.zone.local(2025, 10, 12, 12, 0, 0) do
          time = FeedMonitor::Jobs::CleanupOptions.resolve_time("invalid")
      ...
      test "integer returns nil when value invalid" do
        assert_nil FeedMonitor::Jobs::CleanupOptions.integer("abc")
      end

      test "integer returns integer when valid" do
        assert_equal 50, FeedMonitor::Jobs::CleanupOptions.integer("50")
      end

      test "batch_size returns default when nil or non-positive" do
        assert_equal 100, FeedMonitor::Jobs::CleanupOptions.batch_size({}, default: 100)
        assert_equal 100, FeedMonitor::Jobs::CleanupOptions.batch_size({ batch_size: "0" }, default: 100)
      end

      test "batch_size returns value when positive" do
        assert_equal 25, FeedMonitor::Jobs::CleanupOptions.batch_size({ batch_size: "25" }, default: 100)
      end
    end
  end
end
