    ...
    class BaseTest < ActiveSupport::TestCase
      ...
      test "base class raises when #call is not implemented" do
        assert_raises(NotImplementedError) do
          Base.call(item: FeedMonitor::Item.new, source: FeedMonitor::Source.new)
        end
      end
      ...
      test "settings merge defaults, source overrides, and call overrides" do
        source = FeedMonitor::Source.new(
          scrape_settings: {
            ...
            timeouts: { read: 10 },
            headers: { "User-Agent" => "FeedMonitor" }
          }
        )
        ...
        assert_equal "FeedMonitor", merged_settings[:headers]["User-Agent"]
      ...
      test "result struct exposes keyword attributes" do
        result = FeedMonitor::Scrapers::Base::Result.new(status: :partial, html: nil, content: nil, metadata: {})
