  ...
  module Dashboard
    class RecentActivityPresenterTest < ActiveSupport::TestCase
      test "builds fetch event view model with path" do
        event = FeedMonitor::Dashboard::RecentActivity::Event.new(
          type: :fetch_log,
          id: 42,
        ...
        assert_equal FeedMonitor::Engine.routes.url_helpers.fetch_log_path(42), result[:path]
      end
      ...
      test "builds item event view model with fallbacks" do
        ...
        assert_equal FeedMonitor::Engine.routes.url_helpers.item_path(7), result[:path]
      end
    end
  end
end
