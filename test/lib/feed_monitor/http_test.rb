...
module FeedMonitor
  class HTTPTest < ActiveSupport::TestCase
    ...
    test "merges configured headers and user agent" do
      FeedMonitor.configure do |config|
        config.http.user_agent = -> { "FeedMonitor/Custom" }
        config.http.headers = {
      ...
      client = FeedMonitor::HTTP.client(headers: { "X-Request-ID" => "abc123" })

      assert_equal "FeedMonitor/Custom", client.headers["User-Agent"]
