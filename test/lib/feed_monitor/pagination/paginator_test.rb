  ...
  module Pagination
    class PaginatorTest < ActiveSupport::TestCase
      ...
      test "paginates relation and reports presence of more records" do
        scope = FeedMonitor::Item.order(published_at: :desc)
        result = FeedMonitor::Pagination::Paginator.new(scope:, page: 1, per_page: 5).paginate
      ...
      test "handles out of range and coerces invalid page values" do
        ...
        result_page_zero = FeedMonitor::Pagination::Paginator.new(scope:, page: 0, per_page: 3).paginate
        assert_equal 1, result_page_zero.page
        assert_equal 3, result_page_zero.records.size

        result_page_three = FeedMonitor::Pagination::Paginator.new(scope:, page: 3, per_page: 3).paginate
