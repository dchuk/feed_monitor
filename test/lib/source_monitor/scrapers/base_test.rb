    ...
    class BaseTest < ActiveSupport::TestCase
      ...
      test "base class raises when #call is not implemented" do
        assert_raises(NotImplementedError) do
          Base.call(item: SourceMonitor::Item.new, source: SourceMonitor::Source.new)
        end
      end
      ...
      test "settings merge defaults, source overrides, and call overrides" do
        source = SourceMonitor::Source.new(
          scrape_settings: {
            ...
            timeouts: { read: 10 },
            headers: { "User-Agent" => "SourceMonitor" }
          }
        )
        ...
        assert_equal "SourceMonitor", merged_settings[:headers]["User-Agent"]
      ...
      test "result struct exposes keyword attributes" do
        result = SourceMonitor::Scrapers::Base::Result.new(status: :partial, html: nil, content: nil, metadata: {})
