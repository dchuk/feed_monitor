...
module SourceMonitor
  module Integration
    ...
    test "install generator integrates cleanly into host app" do
      ...
      assert HostAppHarness.exist?("config/initializers/source_monitor.rb"), "initializer was not created"

      routes_contents = HostAppHarness.read("config/routes.rb")
      assert_includes routes_contents, "mount SourceMonitor::Engine, at: \"/source_monitor\""
    end

    test "install generator is idempotent" do
      HostAppHarness.bundle_exec!("rails", "g", "source_monitor:install")
      initializer_snapshot = HostAppHarness.read("config/initializers/source_monitor.rb")

      HostAppHarness.bundle_exec!("rails", "g", "source_monitor:install")

      routes_contents = HostAppHarness.read("config/routes.rb")
      assert_equal 1, routes_contents.scan(/mount SourceMonitor::Engine/).count
      assert_equal initializer_snapshot, HostAppHarness.read("config/initializers/source_monitor.rb")
    end
    ...
    test "install generator preserves host configuration files" do
      ...
      HostAppHarness.bundle_exec!("rails", "g", "source_monitor:install", env: { "REDIS_URL" => "redis://localhost:6379/1" })
    ensure
    ...
    test "install generator skips existing initializer" do
      HostAppHarness.prepare_working_directory do |root|
        ...
        File.write(File.join(initializer_path, "source_monitor.rb"), "# existing initializer")
      end

      HostAppHarness.bundle_exec!("rails", "g", "source_monitor:install")

      content = HostAppHarness.read("config/initializers/source_monitor.rb")
    ...
    test "install generator supports API only hosts" do
      ...
      routes_contents = HostAppHarness.read("config/routes.rb")
      assert_includes routes_contents, "mount SourceMonitor::Engine"
    end
