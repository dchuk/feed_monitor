...
Feedmon installs like any other Rails engine, but it ships enough infrastructure (background jobs, realtime broadcasting, configuration DSL) that it is worth walking through the full setup. This guide assumes you are adding the engine to an existing Rails 8 host application.
...
| --- | --- | --- |
| 1 | `gem "feedmon", github: "dchuk/feedmon"` | Add the engine to your Gemfile |
| 2 | `bundle install` | Install Ruby dependencies |
| 3 | `bin/rails generate feedmon:install --mount-path=/feedmon` | Mount the engine and create the initializer |
| 4 | `bin/rails railties:install:migrations FROM=feedmon` | Copy engine migrations (idempotent) |
...
```ruby
gem "feedmon", github: "dchuk/feedmon"
```
...
```bash
bin/rails generate feedmon:install --mount-path=/feedmon
```

Key outputs:

- Adds `mount Feedmon::Engine, at: "/feedmon"` to your routes (change the path with `--mount-path`)
- Creates `config/initializers/feedmon.rb` with documented configuration defaults
...
Feedmon relies on several tables (sources, items, fetch logs, scrape logs, Solid Cable messages, retention helpers) plus an optional Solid Queue schema. Copy them into your host application with:

```bash
bin/rails railties:install:migrations FROM=feedmon
```
...
Feedmon defaults to Solid Cable for realtime. Rails expects `ApplicationCable::Connection` and `ApplicationCable::Channel` to exist in your host app:
...
Verify `config/cable.yml` allows the adapter you choose. To switch to Redis, update `config/initializers/feedmon.rb` with `config.realtime.adapter = :redis` and provide `config.realtime.redis_url`.
...
Feedmon respects explicit queue adapter overrides. If your host sets `config.active_job.queue_adapter` (for example, to `:inline` or `:sidekiq`), the engine leaves that configuration in place.
...
Open `config/initializers/feedmon.rb` and adjust queue namespaces, HTTP timeouts, scraping adapters, retention limits, authentication hooks, and Mission Control integration to match your environment. The [configuration reference](configuration.md) covers every option with examples.

## 8. Verify the Installation

1. Start your Rails server: `bin/rails server`
2. Ensure a Solid Queue worker is running (see step 6)
3. Visit the mount path (`/feedmon` by default)
...
- Integrate custom scraper adapters or item processors via `Feedmon.configure`
...
| Dedicated Solid Queue database | ✅ Supported | Run `bin/rails solid_queue:install` in the host app before copying Feedmon migrations |
