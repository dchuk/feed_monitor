...
This guide lists common issues you might encounter while installing, upgrading, or operating Feedmon, along with concrete steps to resolve them.

## 1. Mount Path Returns 404

- Ensure the install generator added `mount Feedmon::Engine, at: "/feedmon"` (or your custom path) inside the host `config/routes.rb`.
- Restart the Rails server after modifying routes.
- Confirm your host application routes are reloaded (run `rbenv exec bin/rails routes | grep feedmon`).

## 2. Migrations or Solid Queue Tables Are Missing

- Run `rbenv exec bin/rails railties:install:migrations FROM=feedmon` followed by `rbenv exec bin/rails db:migrate`.
...
- Ensure at least one Solid Queue worker is running; the dashboard reads visibility data via `Feedmon::Jobs::Visibility`.
...
- If job classes cannot load, verify `Feedmon.configure` ran before calling `rake feedmon:cleanup:*`.
...
- Keep `config.mission_control_enabled = true` **and** `config.mission_control_dashboard_path` pointing at that mounted route helper. Call `Feedmon.mission_control_dashboard_path` in the Rails console to confirm it resolves.
...
- Fix by running `npm install` followed by `npm run build` inside the engine root so that `app/assets/builds/feedmon/application.css` and `application.js` exist. The Rake task `app:feedmon:assets:build` wraps the same scripts for CI usage.
...
- Engine version (`Feedmon::VERSION`)
- Host Rails version and Ruby version
- Relevant configuration snippet from `config/initializers/feedmon.rb`
