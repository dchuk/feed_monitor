...
All configuration lives in `Feedmon.configure`—the install generator creates `config/initializers/feedmon.rb` with sensible defaults and inline documentation. This guide expands on every namespace so you know which knobs to turn in production.
...
- Feedmon ships with npm-based bundling via `cssbundling-rails` and `jsbundling-rails`; follow `.ai/engine-asset-configuration.md:11-113` when adjusting dependency versions or adding new entrypoints.
- Keep bundled outputs under the namespaced directories (`app/assets/builds/feedmon`, `images/feedmon`, `svgs/feedmon`) so host apps avoid collisions. See `.ai/engine-asset-configuration.md:32-44` for the layout recommendations.
- Use `rbenv exec bundle exec rake app:feedmon:assets:build` (or `npm run build`) after tweaking Tailwind/Stimulus code to refresh the builds, and rely on `test/dummy/bin/dev` to watch `build:css:watch` + `build:js:watch` during development.
...
- `config.queue_namespace` – prefix applied to queue names (`"feedmon"` by default)
...
Use the helpers exposed on `Feedmon`:

```ruby
Feedmon.queue_name(:fetch)    # => "feedmon_fetch"
Feedmon.queue_concurrency(:scrape) # => 2
```
...
The helper `Feedmon.mission_control_dashboard_path` performs a routing check so the dashboard only renders links that resolve.
...
- `user_agent` – defaults to `Feedmon/<version>`
...
Register adapters that inherit from `Feedmon::Scrapers::Base`:

```ruby
config.scrapers.register(:readability, Feedmon::Scrapers::Readability)
config.scrapers.register(:custom, "MyApp::Scrapers::Premium" )
```

Adapters receive merged settings (`default -> source -> invocation`), and must return a `Feedmon::Scrapers::Result` object. Use `config.scrapers.unregister(:custom)` to remove overrides.
...
- `config.models.table_name_prefix` – override the default `feedmon_` prefix
- `config.models.source.include_concern "MyApp::Feedmon::SourceExtensions"` – mix in concerns before models load
...
config.authentication.authorize_with ->(controller) {
  controller.current_user&.feature_enabled?(:feedmon)
}
...
- `Feedmon.configure` – run-time configuration entry point
- `Feedmon.reset_configuration!` – revert to defaults (useful in tests)
- `Feedmon.events` – direct access to the events registry
- `Feedmon.queue_name(role)` / `Feedmon.queue_concurrency(role)` – convenience helpers
- `Feedmon::Metrics.snapshot` – inspect counters/gauges (great for health checks)
