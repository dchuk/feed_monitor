# Docker Support for Feedmon Examples
...
1. Copy `.env.example` to `.env` and update `APP_PATH` so it points at your generated example directory (relative to the repository root). Example: `APP_PATH=/workspace/examples/feedmon_basic`.
2. From `examples/docker`, run `docker compose up --build`.
3. Visit <http://localhost:3000/feedmon> (basic/custom adapter) or <http://localhost:3000/operations/feedmon> (advanced).
