...
- **Fast feedback:** run `bundle exec rake app:test:smoke` for unit, helper, and job coverage before pushing. Use `FEED_MONITOR_TEST_WORKERS=1` locally when profiling failures for determinism.
...
- **Local commands:**
  - `TAG_PROF=type FEED_MONITOR_TEST_WORKERS=1 bundle exec rails test`
  - `EVENT_PROF=sql.active_record FEED_MONITOR_TEST_WORKERS=1 bundle exec rails test`
  - `TEST_STACK_PROF=1 FEED_MONITOR_TEST_WORKERS=1 bundle exec rails test test/integration`
