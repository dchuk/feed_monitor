...
- **Fast feedback:** run `bundle exec rake app:test:smoke` for unit, helper, and job coverage before pushing. Use `FEEDMON_TEST_WORKERS=1` locally when profiling failures for determinism.
...
- **Local commands:**
  - `TAG_PROF=type FEEDMON_TEST_WORKERS=1 bundle exec rails test`
  - `EVENT_PROF=sql.active_record FEEDMON_TEST_WORKERS=1 bundle exec rails test`
  - `TEST_STACK_PROF=1 FEEDMON_TEST_WORKERS=1 bundle exec rails test test/integration`
